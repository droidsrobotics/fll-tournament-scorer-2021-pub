...
if (isset($_POST['teamNo'])) {
    ...
    $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/rb-rd.txt";

    $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options.");
    fwrite($user, $_POST['store']);
    fclose($user);

    $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/rb-rd-s.txt";

    $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options.");
    fwrite($user, $_POST['score']);
    fclose($user);

    $filename = "../teams/" . $_POST['teamNo'] . "/robotScoresPriv/rd-priv.txt";
    $user = fopen($filename, "w");
    fwrite($user, $_POST['privnotes']);
    fclose($user);
  
    $filename = "../teams/" . $_POST['teamNo'] . "/robotScoresPriv/rd-priv-color.txt";
    $user = fopen($filename, "w");
    fwrite($user, $_POST['colorlabel']);
    fclose($user);

    $filename = "../backup/rubrics/" . $_POST['fname'];
  ...
  try again.  You can load the backup file under advanced options.");
...
<script>
    function tournamentload() {
        ...
        try {
            loadRBsave("<?php echo file_get_contents("../teams/" .$_GET["team"]. "/robotScores/rb-rd.txt"); ?>")
        } catch (err) {
...
<body id="wholeBody">
            ...
            <div data-role="tabs">
                ...
                <div id="tabs-1">
                    <form>
                        ...
                        <div id="robotlist" style="max-width: 1024px;overflow-x: auto;">
                ...
                <div id="tabs-4">
                    ...
                    <form action="" id="serverForm" method="post">
                        ...
                        <h3>Judge's Notes (Private)</h3>
                            <textarea name="privnotes"><?php echo file_get_contents("../teams/" . $_GET['team'] . "/robotScoresPriv/rd-priv.txt"); ?></textarea>
                            <b>Label color:</b>
                            <select data-native-menu="false" data-role="" name="colorlabel" id="colorlabel">
                            ...
                            <script>
                                document.getElementById("colorlabel").value = "<?php echo file_get_contents("../teams/" . $_GET['team']  . "/robotScoresPriv/rd-priv-color.txt"); ?>"
                            </script>
                    </form>
                    <br><br>
 ...
 bottom: -10000px;
 left: -100000px;" type="file" accept=".TDRRD" id="fileInput">
                    ...
                    <script>
                        ...
                        function savetoserver() {
                            ...
                            document.getElementById("score").value = document.getElementById("rdpts").innerHTML
                            ...
                            round = 0
                            score = getRBSaveStr()
                            points = (parseInt(document.getElementById("rdpts").innerHTML)).toLocaleString('en-US', {
                            ...
                            filename = match + "_" + time + "_round-" + round + "_score-" + points + "_" + person + ".TDRRD"
                        ...
                        function loadScore(save) {
                            ...
                            else if (role != "JudgeRD") {
                                if (confirm('This file does not appear to have been created by a Robot Design judge; are you sure you want to load it?')) {
                                    loadRBsave(load);
                                    loadRBsave(load);
