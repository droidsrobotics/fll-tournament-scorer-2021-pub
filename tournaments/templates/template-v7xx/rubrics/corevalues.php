...
if (isset($_POST['teamNo'])) {
    ...
    $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/rb-cv.txt";
    ...
    $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/rb-cv-s.txt";

    $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options.");
    fwrite($user, $_POST['score']);
    fclose($user);

    $filename = "../teams/" . $_POST['teamNo'] . "/robotScoresPriv/cv-priv.txt";
    $user = fopen($filename, "w");
    fwrite($user, $_POST['privnotes']);
    fclose($user);

    $filename = "../teams/" . $_POST['teamNo'] . "/robotScoresPriv/cv-priv-color.txt";
    $user = fopen($filename, "w");
...
<script>
    function tournamentload() {
        ...
        try {
            loadRBsave("<?php echo file_get_contents("../teams/" .$_GET["team"]. "/robotScores/rb-cv.txt"); ?>")
        } catch (err) {
...
<body id="wholeBody">
                ...
                <div id="tabs-4">
                    ...
                    <div style="max-width:500px">
                        ...
                        <form action="" id="serverForm" method="post">
                            ...
                            <textarea name="privnotes"><?php echo file_get_contents("../teams/" . $_GET['team'] . "/robotScoresPriv/cv-priv.txt"); ?></textarea>
                            <b>Label color:</b>
                            ...
                            <script>
                                document.getElementById("colorlabel").value = "<?php echo file_get_contents("../teams/" . $_GET['team']  . "/robotScoresPriv/cv-priv-color.txt"); ?>"
                            </script>
                        </form>
                        <br><br>
 ...
 bottom: -10000px;
 left: -100000px;" type="file" accept=".TDRCV" id="fileInput">
                    ...
                    <script>
                        ...
                        function savetoserver() {
                            ...
                            round = 0
                            score = getRBSaveStr()
                            points = (parseInt(document.getElementById("cvpts").innerHTML)).toLocaleString('en-US', {
                            ...
                            filename = match + "_" + time + "_round-" + round + "_score-" + points + "_" + person + ".TDRCV"
                        ...
                        function loadScore(save) {
                            ...
                            else if (role != "JudgeCV") {
                                if (confirm('This file does not appear to have been created by a Core Values judge; are you sure you want to load it?')) {
                                    loadRBsave(load);
                                    loadRBsave(load);
