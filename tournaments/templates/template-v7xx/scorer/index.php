...
<head>
    ...
    <title>FLL CARGO CONNECT Scorer</title>
    <meta charset="UTF-8" />
...
<body id="wholeBody" onload="initpage()">
    ...
    <div>
        ...
        if (isset($_POST['teamNo'])) {
            ...
            //   echo $_POST['scoreboard'] . "<br>";
            $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/" . $_POST['roundNo'] . ".txt";
            $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options in the scorer.");
            fwrite($user, $_POST['store']);
            fclose($user);

            $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/" . $_POST['roundNo'] . "-gp.txt";
            $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options in the scorer.");
            fwrite($user, $_POST['gppts']);
            fclose($user);

            if ($_POST['scoreboard'] == "on") {
                $filename = "../teams/" . $_POST['teamNo'] . "/pubScores/" . $_POST['roundNo'] . ".txt";
                $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and try again. You can load the backup file under advanced options in the scorer.");
                fwrite($user, explode(";", $_POST['store'])[0]);
                fclose($user);
            }


            $filename = "../teams/" . $_POST['teamNo'] . "/robotScoresPriv/". $_POST['roundNo'] ."-priv.txt";
            $user = fopen($filename, "w");
            fwrite($user, $_POST['privnotes']);
            fclose($user);

            $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/". $_POST['roundNo'] ."-pub.txt";
            $user = fopen($filename, "w");
            fwrite($user, $_POST['pubnotes']);
            fclose($user);

            $filename = "../backup/scores/" . $_POST['fname'];
            $user = fopen($filename, "w") or die("Failed to save. Please download the backup file by clicking

  <button onclick='download(\"" . $_POST['bkp'] . "\", \"" . $_POST['fname'] . "\", \"txt\");'>here</button> and
  
  try again.  You can load the backup file under advanced options in the scorer.");
            fwrite($user, $_POST['bkp']);
            fclose($user);

            echo "<b style='color: red'>Saved score</b><script>alert('Saved Score')</script>";
        }
        ?>
        ...
        <script>
            function tournamentload() {
                ...
                loadsave(("<?php echo file_get_contents("../teams/" . $_GET["team"] . "/robotScores/" .  $_GET["round"] . ".txt");?>").split(';')[1])
                loadsave(("<?php echo file_get_contents("../teams/" . $_GET["team"] . "/robotScores/" .  $_GET["round"] . ".txt");?>").split(';')[1])
                ...
                <tr>
                    ...
                    <script>
                        ...
                        if (getParameterByName('team') != null) {
                            ...
                            document.write('<br><text>Round No.</text>: ' + getParameterByName('round') + '</b>')
                    ...
                    <td style="width:50%; font-size:24px; text-align:center">
                        <b id="title">FLL CARGO CONNECT Scorer</b>
                    </td>
                    <td style="width:25%; font-size:12px; text-align:right">
                        <!-- <img alt="FLL..." style="border:1px solid #021a40;" src="assets/images/FLL.png" height="57"> <img alt="INTO ORBIT..." style="border:1px solid #021a40;" src="assets/images/replay.jpg" height="57">  -->
                        <img alt="FLL Tutorials" style="border:1px solid #021a40;" src="assets/images/flltutorials.png" width="139"><img class="only-mobile" alt="FLL Tutorials" style="border:1px solid #021a40;" src="assets/images/flltutorials.png" width="139">
                    </td>
                </tr>
            </table>
                ...
                <div id="tabs-1">
                    ...
                    <div style="max-width:500px">
                        ...
                        <form action="" id="serverForm" method="post">
                            ...
                            <input type="hidden" id="gppts" name="gppts">

                            <h3>Referee's Notes (Shared)</h3>
                            <textarea name="pubnotes"><?php echo file_get_contents("../teams/" . $_GET['team'] . "/robotScores/".$_GET['round'] ."-pub.txt"); ?></textarea>


                            <h3>Referee's Notes (Private)</h3>
                            <textarea name="privnotes"><?php echo file_get_contents("../teams/" . $_GET['team'] . "/robotScoresPriv/".$_GET['round'] ."-priv.txt"); ?></textarea>
                            
                           
                            <input type="checkbox" id="scoreboard" name="scoreboard" checked="checked">
                            <label for="scoreboard"> Save to Scoreboard</label><br>
                        </form>
                        <button onclick="savetoserver();" id="saverTscorer" type="button">Save Score</button>
                        <br><br><br><br><br>
                        <input onchange="" style=" position: fixed;
 bottom: -10000px;
 left: -100000px;" type="file" accept=".TDRCC" id="fileInput">

                        Advanced:
                        <button data-role="none" id="importText" onclick=" document.getElementById('fileInput').click()">Load Backup Score from File</button>
                        <br><br>
                    </div>
                </div>
...
<script>
    ...
    function savetoserver() {
        ...
        document.getElementById("gppts").value = graciousProfessionalism;
        ...
        score = String(getvar())
        points = (parseInt(document.getElementById("allpoints").innerHTML)).toLocaleString('en-US', {
        ...
        filename = match + "_" + time + "_round-" + round + "_score-" + points + "_" + person + ".TDRCC"
    ...
    function loadScore(save) {
        ...
        if (team != teamC || round != roundC) alert("Incorrect team or round")
        else if (role != "Referee") {
            if (confirm('This file does not appear to have been created by a referee; are you sure you want to load it?')) {
                loadsave(load);
                loadsave(load);
